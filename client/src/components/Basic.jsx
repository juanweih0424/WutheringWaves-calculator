...
export default function Basic() {
  ...
  const title = current?.basic?.name ?? "Normal Attack";
  const baseDetail = current?.basic?.detail ?? null;

  const detailMap = useDetailMap(current);
  const basicExtensions = useMemo(
    () => collectExtensions(activeChain, detailMap, "basic"),
    [activeChain, detailMap]
  );
  const { ability, ratioMeta } = useMemo(
    () => mergeAbilityWithExtensions(baseDetail, basicExtensions, "basic"),
    [baseDetail, basicExtensions]
  );
  ...
  const rowsWithRatios = useMemo(
    () => applyRatioScaling(rowsWithMods, ratioMeta, "basic"),
