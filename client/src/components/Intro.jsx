...
export default function Intro() {
  ...
  const title = current?.intro?.name ?? "Intro Skill";
  const baseDetail = current?.intro?.detail ?? null;

  const detailMap = useDetailMap(current);
  const introExtensions = useMemo(
    () => collectExtensions(activeChain, detailMap, "intro"),
    [activeChain, detailMap]
  );
  const { ability, ratioMeta } = useMemo(
    () => mergeAbilityWithExtensions(baseDetail, introExtensions, "intro"),
    [baseDetail, introExtensions]
  );
  ...
  const rowsWithRatios = useMemo(
    () => applyRatioScaling(rowsWithMods, ratioMeta, "intro"),
