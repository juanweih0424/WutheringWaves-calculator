...
export default function CharModal({ open, onClose, onSelect }) {
  ...
  const elementOptions = useMemo(() => {
    ...
    (items ?? []).forEach((r) => {
      if (r?.element) unique.add(r.element);
    });
  ...
  const weaponOptions = useMemo(() => {
    const unique = new Set();
    (items ?? []).forEach((r) => {
      if (r?.weapon) unique.add(r.weapon);
    });
    return Array.from(unique).sort();
  }, [items]);
  const rarityOptions = useMemo(() => {
    const unique = new Set();
    (items ?? []).forEach((r) => {
      if (r?.rarity) unique.add(Number(r.rarity));
    });
  ...
  }, [items, searchTerm, elementFilter, rarityFilter, weaponFilter]);
          ...
          <div className="mb-5 space-y-4">
              ...
              <div className="flex items-center lg:gap-2">
                ...
                {elementOptions.map((el) => {
                  ...
                  return (
                    ...
                    className={`flex items-center justify-center rounded-full p-0.5 transition cursor-pointer ${
                ...
                {rarityOptions.map((rarity) => {
                  ...
                  return (
                    <button
                      ...
                      className={`flex items-center justify-center rounded-full border px-3 py-1 cursor-pointer text-xs lg:text-sm transition ${
                        active
            ...
            {weaponOptions.length > 0 && (
              <div className="flex flex-wrap items-center gap-2">
                <span className="text-xs lg:text-sm uppercase tracking-wide opacity-60">Weapon</span>
