...
export default function Skill() {
  ...
  const rows = useMemo(() => {
    ...
    const detail = current?.skill?.detail || {};
    return Object.entries(detail).map(([label, row]) => {
      const mv = talentMv(1, 10, row.base, row.max, skill); 
  ...
  }, [current?.id, current?.skill?.detail, totals, skill, elementKey,enemylvl, enemyres]);

  return current ? (
    <div className="p-4 grid gap-2 border-0 shadow-xl">
      <h2 className="text-lg font-bold">{current.skill?.name ?? "Resonance Skill"}</h2>
      {(rows ?? []).map((r) => (
        <div
          key={r.key}
