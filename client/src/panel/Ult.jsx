...
export default function Ult() {
  ...
  const rows = useMemo(() => {
    ...
    const detail = current?.ult?.detail || {};
    return Object.entries(detail).map(([label, row]) => {
      const mv = talentMv(1, 10, row.base, row.max, ult); 
  ...
  }, [current?.id, current?.ult?.detail, totals, ult, elementKey, enemylvl, enemyres]);

  return current ? (
    <div className="p-4 grid gap-2 border-0 shadow-xl">
      <h2 className="text-lg font-bold">{current.ult?.name ?? "Resonance Liberation"}</h2>
      {(rows ?? []).map((r) => (
        <div
          key={r.key}
