...
export default function Intro() {
  ...
  const rows = useMemo(() => {
    ...
    const detail = current?.forte?.detail || {};
    return Object.entries(detail).map(([label, row]) => {
      const mv = talentMv(1, 10, row.base, row.max, forte); 
  ...
  }, [current?.id, current?.forte?.detail, totals, forte, elementKey,enemylvl, enemyres]);

  return current ? (
    <div className="p-4 grid gap-2 border-0 shadow-xl">
      <h2 className="text-lg font-bold">{current.forte?.name ?? "Forte Circuit"}</h2>
      {(rows ?? []).map((r) => (
        <div
          key={r.key}
